...
interface Artwork {
  title: string;
  type: string; // Required by ArtCardPerArtist
...
const ViewArtistProfileAdmin = () => {
  ...
  // Fetch artworks from ARTIST_PROFILE
  const artistArtworks: Artwork[] =
    ARTIST_PROFILE.find((artist: Artist) => artist.Name === artistData.name)
      ?.ARTIST_DATA.ArtWork || [];
      ...
      <div className="bg-[#FAF9F6]/50 backdrop-blur-xl w-full max-w-7xl rounded-3xl shadow-2xl p-10 lg:p-16 relative">
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-start gap-12 relative">
          ...
          >
            ...
            <div className="mt-8 flex flex-col justify-center gap-2">
              ...
              >
                {artistData.artisttype.toUpperCase()}
              </motion.button>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {artistArtworks.map((art: Artwork, index: number) => (
              <ArtCardPerArtist
                key={index}
...
export default ViewArtistProfileAdmin;